...
$(document).ready(function () {
    ...
    cutTextBySelector('.header__category-name', 25);
...
/**
 * Метод обрезает текст
 * @param elem принимает селектор с текстовым элементом или элементами
 * @param count количество символов, после которого будет обрезаться текст
 */
function cutTextBySelector(element, countLimit) {
  $(element).each(function(idx, elem) {
    ...
    if (lengthText > countLimit) {
      for (let s = countLimit; s > 0; s--) {
        if (text.charAt(s - 1) === ' ' ||
            text.charAt(s - 1) === ',' ||
            text.charAt(s - 1) === '.' ||
            text.charAt(s - 1) === ';') {
